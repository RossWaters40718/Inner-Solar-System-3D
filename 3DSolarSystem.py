...
def anim_start():# Start Animation
    ...
    Anim=animation.FuncAnimation(fig=Anim_Fig,func=anim_advance,repeat=False,frames=len(Epoch), 
                                 blit=True,interval=Anim_Speed.get(),cache_frame_data='False')
def set_defaults():
...
def start():# Start Animation
    ...
    try:
        ...
        if Increment.get()<= 0.0:# Validate Time Increment
            ...
            return
        if Duration.get()!=Old_Duration.get() or Start_Date.get()!=Old_StartDate.get() or Increment.get()!=Old_Increment.get():# Something Changed
